...
const [image, setImage] = useState([]);
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:4000/api/get-posts", )
...
const showPostDetails = 
  ...
  const handleDelete = async() => {
        ...
        axios.delete("http://localhost:4000/api/deletepost/:{post_id}", {
        ...
        <div>
            {post &&
                        post.slice(0, 10).map((post, index) => (
                            ...
                            <div class='flex justify-left bg-white flex-row mb-[5px] py-[5px] text-deepBlue text-left text-[15px] border-[2px] w-full border-gray2 border-style-solid lg:text-[15px] font-medium lg:font-medium lg:leading-[20px] leading-[20px] '>
                            <div class='line-clamp-1 mx-[5px] lg:mx-[20px] mt-[5px] w-full pl-[5px] lg:pl-[20px] '>
                            {post.headline} 
                            </div>  
                             
                            <div onClick={() => {showPostDetails(post.post_id)}} class='bg-deepBlue text-white text-[15px] grid content-center px-[20px] mx-[2px]  '>
                                Edit
                            </div>  
                            <div onClick={handleDelete} class='bg-red text-white text-[15px]  px-[20px]  grid content-center mx-[2px] '>
                                Delete
                            </div>              
                        </div>                        
